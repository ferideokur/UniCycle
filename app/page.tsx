    ...
    <main className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
          ...
          <div className="flex justify-between items-center h-20 gap-6">
            <div className="flex-shrink-0">
              <Link href="/" className="hover:scale-105 transition-transform inline-block">
                <Image src="/logo.png.jpeg" alt="UniCycle Logo" width={90} height={90} className="object-contain mix-blend-multiply contrast-125 py-1" priority />
              </Link>
            </div>
            ...
            <div className="flex items-center gap-5">
              ...
              {user ? (
                <>
                  ...
                  <button onClick={handleLogout} className="text-slate-400 hover:text-red-500 font-bold transition-colors text-sm">Çıkış</button>
                </>
              ) : (
                <Link href="/login" className="flex items-center gap-2 bg-slate-800 text-white px-6 py-2.5 rounded-full font-bold hover:bg-black transition-colors">Giriş Yap</Link>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mt-8 flex gap-8 items-start">
        ...
        <aside className="w-72 hidden lg:block sticky top-28 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
          <h3 className="font-black text-lg text-slate-800 mb-5 pl-2">Kategoriler</h3>
          
          <ul className="space-y-1">
            <li>
              <button 
                onClick={() => { setExpandedGroup(null); setActiveFilter("TÜMÜ"); }}
              ...
              return (
                <li key={mainCat} className="mb-1">
                  ...
                  >
                    ...
                    <span className={`text-xs transition-transform duration-300 ml-2 ${isExpanded ? 'rotate-90 text-blue-600' : 'text-slate-400'}`}>▶</span>
                  </button>
        ...
        <section className="flex-1 min-w-0">
          ...
          <div className="block lg:hidden mb-6">
            <div className="flex overflow-x-auto custom-scrollbar gap-2 pb-2">
              <button onClick={() => { setExpandedGroup(null); setActiveFilter("TÜMÜ"); }} className={`whitespace-nowrap px-5 py-2.5 rounded-full font-bold text-sm ${activeFilter === "TÜMÜ" ? "bg-blue-600 text-white" : "bg-white text-slate-600 border border-slate-200"}`}>Tümü</button>
              {Object.keys(CATEGORY_MAP).map((cat) => (
                <button key={cat} onClick={() => handleMainCategoryClick(cat)} className={`whitespace-nowrap px-5 py-2.5 rounded-full font-bold text-sm ${expandedGroup === cat ? "bg-blue-600 text-white" : "bg-white text-slate-600 border border-slate-200"}`}>
                  {cat}
                </button>
              ))}
            </div>
            {expandedGroup && CATEGORY_MAP[expandedGroup] && (
              <div className="flex overflow-x-auto custom-scrollbar gap-2 mt-2 pb-2 animate-in fade-in">
                {CATEGORY_MAP[expandedGroup].map((subCat) => (
                  <button key={subCat} onClick={() => setActiveFilter(subCat)} className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${activeFilter === subCat ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
                    {subCat}
                  </button>
                ))}
              </div>
          ...
          {activeFilter === "TÜMÜ" && searchTerm === "" && (
            <div className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 rounded-[2.5rem] p-8 md:p-12 mb-10 text-white shadow-xl relative overflow-hidden flex items-center">
              <div className="relative z-10 max-w-lg">
                <span className="bg-white/20 backdrop-blur-sm text-white font-black px-4 py-1.5 rounded-full text-xs mb-4 inline-block tracking-widest">YENİ DÖNEM BAŞLIYOR</span>
                <h2 className="text-4xl md:text-5xl font-black mb-4 leading-tight tracking-tight">Vizeler Yaklaşıyor! 📚</h2>
                <p className="text-lg font-medium opacity-90 mb-6 leading-relaxed">Üst dönemlerin ders notlarını, çıkmış sorularını ve kitaplarını hemen keşfet. Hazır notlar vitrinde seni bekliyor.</p>
          ...
          <div className="flex justify-between items-end mb-6 pb-4">
            <h3 className="text-2xl font-black text-slate-800 tracking-tight">
              {searchTerm ? `"${searchTerm}" araması` : activeFilter === "TÜMÜ" ? "Kampüsün En Yenileri" : `${activeFilter}`}
            </h3>
          ...
          {isLoading ? (
            <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100 shadow-sm mt-4">
              <span className="animate-spin text-5xl block mb-4">⏳</span>
              <h3 className="text-2xl font-black text-slate-800 mb-2">Vitrin Yükleniyor...</h3>
              <p className="text-slate-500 font-medium text-lg">Kampüsteki en taze ilanlar veritabanından getiriliyor.</p>
            </div>
          ) : filteredProducts.length === 0 ? (
            /* 🕵️‍♀️ BOŞ EKRAN */
            <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100 shadow-sm mt-4">
              <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center text-5xl mx-auto mb-6">🕵️‍♀️</div>
              <h3 className="text-2xl font-black text-slate-800 mb-3">Buralar biraz ıssız...</h3>
              <p className="text-slate-500 font-medium text-lg mb-8 max-w-md mx-auto">Sanki kampüste kimse bu kategoride bir şey satmıyor. İlk adımı sen atmaya ne dersin?</p>
                    ...
                    <div className="aspect-[4/5] relative overflow-hidden bg-slate-100">
                      ...
                      <button 
                        ...
                        className={`absolute top-3 right-3 p-2.5 rounded-full shadow-md backdrop-blur-md transition-all hover:scale-110 active:scale-95 z-10 ${
                          isLiked ? 'bg-red-500/90 text-white' : 'bg-white/90 text-gray-400 hover:text-red-500'
                        }`}
                      >
