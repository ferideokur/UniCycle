  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isLogin) {
      ...
      if (nameParts.length < 2) {
        setMessage("❌ Lütfen adınızı ve soyadınızı aralarında boşluk bırakarak tam yazın (Örn: Feride Okur).");
        return; 
      }
      ...
      try {
        ...
        if (response.ok) {
          setMessage("✅ " + text); 
          setFullName(""); setEmail(""); setPassword(""); setCustomUniversity("");
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      ...
      <div className="absolute top-8 left-8">
        <Link href="/" className="text-gray-400 hover:text-blue-600 font-bold flex items-center gap-2 transition-colors">
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl overflow-hidden border border-gray-100 mt-12 mb-12">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-8 text-center border-b border-gray-100">
          <div className="flex justify-center mb-4">
            <Image src="/logo.png.jpeg" alt="UniCycle Logo" width={80} height={80} className="object-contain mix-blend-multiply contrast-125 brightness-105" priority />
          </div>
          <h1 className="text-2xl font-black text-blue-900">
            {isLogin ? "Tekrar Hoş Geldin! 👋" : "UniCycle'a Katıl! 🚀"}
          </h1>
          <p className="text-gray-500 text-sm mt-2 font-medium">
            {isLogin ? "Kampüs pazaryerine giriş yap." : "Öğrenci e-postan ve okulunla saniyeler içinde kayıt ol."}
          </p>
        </div>

        <div className="p-8">
          {message && (
            <div className={`mb-4 p-3 rounded-xl text-sm font-bold text-center ${message.includes("✅") ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
              {message}
            </div>
          )}
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {!isLogin && (
              <>
                {/* 🚀 İSİM SOYİSİM */}
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">İsim ve Soyisim</label>
                  <input 
                    ...
                    required={!isLogin} 
                  />
                  <p className="text-xs text-gray-400 mt-1 font-medium">Güvenlik için soyadınızı girmek zorunludur.</p>
                </div>

                {/* 🚀 ÜNİVERSİTE SEÇİMİ */}
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Üniversiteniz</label>
                  <select 
                ...
                {university === "Diğer..." && (
                  <div className="animate-fade-in">
                    <label className="block text-sm font-bold text-blue-700 mb-1.5">Okulunuzun Adı</label>
                    <input 
                      type="text" value={customUniversity} onChange={(e) => setCustomUniversity(e.target.value)}
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5">Üniversite E-Postası</label>
              <input 
            ...
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black text-lg py-3.5 rounded-xl transition-colors shadow-md mt-4">
          ...
          <div className="mt-8 text-center text-sm font-medium text-gray-600">
            ...
            <button type="button" onClick={() => { setIsLogin(!isLogin); setMessage(""); }} className="ml-1.5 font-black text-blue-600 hover:text-blue-800 transition-colors">
