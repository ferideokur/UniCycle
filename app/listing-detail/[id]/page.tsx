  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans relative w-full overflow-x-hidden flex flex-col">
      ...
      <header className="bg-white/90 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-gray-100 flex flex-col">
        <div className="max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex justify-between items-center gap-2 sm:gap-6 pt-1 sm:pt-0">
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-4 shrink-0">
            ...
            {currentUser ? (
              ...
              <Link
                href="/login"
                className="bg-slate-800 text-white px-5 py-2.5 rounded-full font-bold hover:bg-black transition-colors text-xs sm:text-sm shrink-0"
              >
                Giriş Yap
              </Link>
            )}
        ...
        {/* Mobil Arama */}
        <div className="md:hidden pb-3 pt-1 w-full relative z-40 px-4 bg-white border-b border-gray-100 shadow-sm">
          <form onSubmit={handleSearchSubmit} className="w-full relative flex gap-2">
            <div className="flex-1 relative">
              <input
                type="text"
                placeholder="Ürün, kategori veya ders notu ara..."
                className="w-full bg-[#F3F4F6] text-slate-800 rounded-lg py-2.5 px-4 pl-10 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all border border-transparent font-medium text-sm"
                value={searchTerm}
                onChange={(e) => {
                  setSearchTerm(e.target.value);
                  setIsDropdownOpen(true);
