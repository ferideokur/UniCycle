  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        const cleanQuery = isUserSearch ? rawQuery.substring(1).trim() : rawQuery.trim();
        if (!cleanQuery) {
          setIsLoading(false);
          return;
        }
        ...
        if (isUserSearch) {
          setSearchType("users");
          const res = await fetch(`http://localhost:8080/api/users/search?q=${encodeURIComponent(cleanQuery)}`);
          if (res.ok) {
          ...
          const res = await fetch(`http://localhost:8080/api/products/search?q=${encodeURIComponent(cleanQuery)}`);
          if (res.ok) {
            const data = await res.json();
...
<Link href="/" className="inline-flex items-center gap-3 hover:scale-105 transition-transform group">
<Image src="/logo.jpeg" alt="UniCycle İkon" width={48} height={48} className="rounded-md shadow-sm" priority />
...
<input
              ...
              />
<button type="submit" className="absolute right-1.5 top-1.5 bottom-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 rounded-full text-sm transition-colors">
                Ara
</button>
</form>
</div>
...
<Link href="/create-listing" className="font-bold text-blue-600 hover:text-blue-700 transition-colors hidden md:block">
              + İlan Ver
</Link>
            {user ? (
...
<button onClick={() => setIsNotificationOpen(!isNotificationOpen)} className="relative w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 hover:bg-slate-200 transition-colors" title="Bildirimler">
...
<Link href="/notifications" onClick={() => setIsNotificationOpen(false)} className="block w-full text-center px-4 py-3 bg-slate-50 text-xs font-bold text-blue-600 hover:bg-slate-100 transition-colors">
                ...
                <Link href="/profile" className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-full font-bold hover:bg-blue-700 shadow-sm hover:shadow-md transition-all ml-2">
...
<Link href="/login" className="flex items-center gap-2 bg-slate-800 text-white px-6 py-2.5 rounded-full font-bold hover:bg-black transition-colors text-sm">
                Giriş Yap
</Link>
            )}
</div>
...
<div>
<h1 className="text-3xl font-black text-slate-800 tracking-tight">Arama Sonuçları</h1>
<p className="text-lg text-slate-500 mt-2 font-medium">
<span className="font-bold text-blue-600">"{rawQuery}"</span> araması için {isLoading ? "..." : results.length} sonuç bulundu.
</p>
</div>
<Link href="/" className="hidden md:flex items-center text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors">
&larr; Anasayfaya Dön
</Link>
</div>
...
<div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center text-5xl mx-auto mb-6">🕵️‍♀️</div>
<h3 className="text-2xl font-black text-slate-800 mb-3">Aradığın şeyi bulamadık...</h3>
<p className="text-slate-500 font-medium text-lg mb-8 max-w-md mx-auto">
              Görünüşe göre kampüste henüz kimse "<span className="font-bold text-slate-700">{rawQuery}</span>" ile ilgili bir şey paylaşmamış.
</p>
<Link href="/" className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-3.5 px-8 rounded-full transition-colors inline-block">
              Tüm İlanlara Göz At
</Link>
</div>
        ) : (
...
<div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            ...
            {searchType === "users" && results.map((userItem: any) => (
<div key={userItem.id} className="group bg-white rounded-3xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 flex flex-col items-center text-center">
...
<h2 className="text-lg font-bold text-slate-800">{userItem.fullName}</h2>
<p className="text-sm font-medium text-slate-400 mt-1">@{userItem.fullName ? userItem.fullName.split(" ")[0].toLowerCase() : "user"}</p>
<Link href={`/user/${userItem.id}`} className="mt-6 bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-600 font-bold py-2 px-6 rounded-full transition-colors w-full block text-center">
                  Profili Gör
</Link>
</div>
            ))}
            ...
            {searchType === "products" && results.map((p: any) => {
              const isLiked = likedProducts.includes(p.id);
 
              return (
<Link href={`/listing-detail/${p.id}`} key={p.id} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 flex flex-col">
...
<div className="w-full h-full flex items-center justify-center text-6xl">📦</div>
                    ...
                    {p.priceType === "takas" && <div className="absolute top-3 left-3 bg-purple-600 text-white text-[10px] font-black px-2.5 py-1.5 rounded-lg shadow-md uppercase">Takas</div>}
                    {p.priceType === "ucretsiz" && <div className="absolute top-3 left-3 bg-green-500 text-white text-[10px] font-black px-2.5 py-1.5 rounded-lg shadow-md uppercase">Ücretsiz</div>}
                    {/* 🚀 EKLENEN KISIM: Kalp Butonu ve Bildirim Gönderme */}
<button onClick={(e) => toggleLike(e, p)} className={`absolute top-3 right-3 p-2.5 rounded-full shadow-md backdrop-blur-md transition-all hover:scale-110 active:scale-95 z-10 ${isLiked ? "bg-red-500/90 text-white" : "bg-white/90 text-gray-400 hover:text-red-500"}`}>
...
<Suspense fallback={<div className="text-center font-bold text-slate-500 text-xl mt-40 animate-pulse">Sayfa Yükleniyor...</div>}>
<SearchContent />
</Suspense>
</main>
  );
