    ...
    <div className="max-w-[1400px] mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          ...
          {searchType === "users" && results.map((user: any) => (
            <div key={user.id} className="group bg-white rounded-3xl p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 flex flex-col items-center text-center">
              ...
              <p className="text-sm font-medium text-slate-400 mt-1">@{user.fullName.split(" ")[0].toLowerCase()}</p>
              
              {/* 🚀 DEĞİŞTİRİLEN KISIM: Button yerine Link etiketi geldi */}
              <Link href={`/user/${user.id}`} className="mt-6 bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-600 font-bold py-2 px-6 rounded-full transition-colors w-full block text-center">
                Profili Gör
              </Link>
            </div>
          ))}
...
export default function SearchPage() {
  return (
    <main className="min-h-screen bg-[#F8FAFC] py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <Suspense fallback={<div className="text-center font-bold text-slate-500 text-xl mt-20 animate-pulse">Loading search results...</div>}>
        <SearchContent />
      </Suspense>
    </main>
  );
